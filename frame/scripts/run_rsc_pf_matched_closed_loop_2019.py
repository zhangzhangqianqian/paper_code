...
from src.joint_dispatch.matched_closed_loop_providers import build_external_provider, load_frozen_rsc_pf_provider
...
def validate_config(config: Mapping[str, Any], base_dir: Path = FRAME_ROOT) -> None:
    ...
    for key in ("selection_file", "training_file", "rsc_pf_contract", "rsc_pf_run_root", "rsc_pf_pilot_receipt", "rsc_pf_legacy_rollout", "benchmark_path", "capacity_receipt_path", "external_config", "external_source_receipt", "external_output_root"):
...
def _rsc_normalization(train) -> Any:
    stats = ExternalV46Normalization.fit(train)
    return SimpleNamespace(field_mean={"load": stats.load_mean, "exog": stats.exog_mean, "device": stats.device_mean}, field_scale={"load": stats.load_scale, "exog": stats.exog_scale, "device": stats.device_scale}, train_years=(2015, 2016, 2017, 2018))
...
def _write_atomic_row(row_dir: Path, result: MatchedClosedLoopResult, receipt: Mapping[str, Any], *, resume: bool) -> None:
    if row_dir.is_dir():
        if not resume:
            raise FileExistsError(f"completed row exists: {row_dir}")
        return
    writing = row_dir.with_name(row_dir.name + ".writing")
    if writing.exists():
...
def _selection_and_parameters(config: Mapping[str, Any]):
    ...
    return selection, train, _parameters(benchmark, capacity), _rsc_normalization(train), ExternalV46Normalization.fit(train)
...
def run_matrix(config_path: str | Path, *, methods: tuple[str, ...] = METHOD_ORDER, seeds: tuple[int, ...] | None = None, limit: int | None = None, warmup_origins: int | None = None, stop_after: int | None = None, output_root: str | Path | None = None, resume: bool = False) -> dict[str, Any]:
    ...
    for method in methods:
        ...
        for seed in row_seeds:
            ...
            _write_atomic_row(row_dir, result, receipt, resume=resume)
...
def main() -> int:
    ...
    if args.verify_rsc_pf_legacy_only:
        raise SystemExit("legacy-only replay is exposed after the provider reconstruction; use the replay API before opening the matched matrix")
