"""Frozen scientific boundary for the formal-v4.5 training repair."""
...
SCHEMA_VERSION = "joint-forecast-dispatch-formal-v4.5"
_PILOT_THRESHOLDS = {
    "minimum_leakage_reduction": 0.30,
    "maximum_active_wape_relative_degradation": 0.05,
    "maximum_electricity_gas_wape_relative_degradation": 0.02,
...
class FormalV45Contract:
    ...
    def validate(self) -> None:
        if self.payload.get("schema_version") != SCHEMA_VERSION:
            raise ValueError("formal-v4.5 schema required")
        if self.payload.get("protocol_status") != "frozen":
            raise ValueError("formal-v4.5 contract is not frozen")
        if self.train_years != (2015, 2016, 2017, 2018) or self.selection_year != 2019:
            raise ValueError("formal-v4.5 training and selection years are invalid")
        if self.evaluation_year != 2020 or self.excluded_years != (2021,):
            raise ValueError("formal-v4.5 evaluation boundary is invalid")
        ...
        if self.payload.get("lookback") != 24 or self.payload.get("horizon") != 4:
            raise ValueError("formal-v4.5 requires a 24-hour history and four-hour horizon")
        if self.control_dim != 15 or self.dispatch_dim != 21:
            raise ValueError("formal-v4.5 control dimensions are invalid")
        if self.payload.get("allow_future_binary_decisions") is not False:
            raise ValueError("future binary decisions are outside formal-v4.5")
        ...
        if self.methods != METHODS:
            raise ValueError("formal-v4.5 method matrix is incomplete or reordered")
...
def load_formal_v4_5_contract(path: str | Path) -> FormalV45Contract:
    source = Path(path).resolve()
    raw = source.read_bytes()
    payload = json.loads(raw)
    if not isinstance(payload, Mapping):
        raise ValueError("formal-v4.5 contract must be a JSON object")
    contract = FormalV45Contract(MappingProxyType(dict(payload)), hashlib.sha256(raw).hexdigest(), source)
    contract.validate()
    return contract


__all__ = ["FormalV45Contract", "SCHEMA_VERSION", "load_formal_v4_5_contract"]
