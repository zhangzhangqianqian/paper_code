# RSC-PF Formal-v4 Gate 0 Comprehensive Repair Design
...
Repair the formal-v4 experiment foundation so that every artifact entering Gate 1 is causally generated, bound to the frozen 2015--2018 training protocol, physically interpretable, reproducible from immutable inputs, and verified by a genuinely fail-closed Gate 0.
...
Before rebuilding data, all formal-v4 code, configuration, tests, third-party source receipts, and repair documentation must be committed. Gate 0 records the Git commit and refuses a dirty worktree affecting formal-v4 inputs. Unrelated user changes may remain dirty only if the Gate 0 manifest proves that none of their paths are consumed by formal-v4.

The protocol freeze must be parsed and validated, not merely checked for existence. Its stored hashes must equal the current files. The benchmark, parameter ledger, iTransformer receipt and imported upstream source hashes, Differentiable-LP lock, state-feature ledger, search budget, and all Gate 0 scripts must be included in the source manifest.

## 6. Formal-v4 Standard IES Rebuild
...
The capacity audit remains training-only but no longer takes the first 500 chronological windows. A deterministic, precomputed origin manifest selects 500 valid origins from 2015--2018 with coverage of:
...
Each candidate is evaluated using the resolved formal-v4 benchmark, correctly scaled PV/WT availability, identical initial-state rules, and the common PI-MPC solver. The smallest multiplier satisfying cooling shortage-energy ratio at most 0.5% and shortage-hour rate at most 1.0% becomes `capacity_adequate_main`; multiplier 1.0 remains `benchmark_capacity_stress`.

The capacity receipt contains the origin-manifest hash, resolved parameter hash, per-stratum results, candidate-level results, selected multiplier, solver version, and deterministic scenario identity. Runtime measurements are stored separately and are excluded from scientific identity hashes.
...
Selection-year states are generated only after the training trajectory and normalization are frozen. The selection trajectory may carry the final causal state across the year boundary, but selection labels must never affect training normalization, benchmark construction, capacity selection, or `C_ref`.
...
Stage P remains prediction pretraining, Stage S remains scheduler imitation with the forecaster frozen, and Stage J remains the formal joint/decoupled comparison. The exact curriculum, including the final imitation weight, must be stated once in the protocol and asserted by tests; code defaults may not silently override it.
...
- protocol and source manifest match the frozen Git commit;
- formal-v4 benchmark is derived only from 2015--2018;
- capacity audit has representative nonzero cooling coverage and a valid selection;
...
- no 2020/evaluation artifact was opened or created;
...
The successful receipt lists every planned issue identifier, measured evidence, artifact paths and hashes, test counts, resource projections, timestamps and the explicit statement `test_set_accessed=false` supported by an access log.
...
- all formal-v4 inputs and code are committed and hash-recorded;
- a new 2015--2018 benchmark receipt exists;
- the representative capacity audit produces an interpretable nonzero-demand result;
...
- an independent post-run audit reproduces every hash and check without reading 2020.
